  ...
  <section id="education" className="relative py-20 md:py-32 px-4">
    <div className="container mx-auto max-w-3xl">
      ...
      <div className="space-y-4">
        {linkedinProfile.education.map((item, index) => (
          <ScrollReveal key={`${item.school}-${item.degree}`} direction="up" scale={0.95} delay={index * 0.08}>
            <GlassCard className="p-6 md:p-8 hover:bg-primary/5 transition-colors duration-300">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  <h3 className="font-display font-semibold text-foreground text-lg">{item.degree}</h3>
                  ...
                  <p className="text-xs text-muted-foreground mt-3 leading-relaxed">{item.focus}</p>
                </div>
              </div>
            </GlassCard>
          </ScrollReveal>
